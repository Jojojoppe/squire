...
static void error1(int mode, const char *fmt, va_list ap)
{
    ...
    if (mode == ERROR_WARN)
        strcat_printf(buf, sizeof(buf), ANSI_COLOR_YELLOW "warning: " ANSI_COLOR_RESET);
    else
        strcat_printf(buf, sizeof(buf), ANSI_COLOR_RED "error: " ANSI_COLOR_RESET);
...
static int tcc_set_linker(TCCState *s, const char *option)
{
    ...
    while (*option) {
        ...
        // SQUIRE ADDITION
        } else if (link_option(option, "physoffset=", &p)) {
            s->output_physoffset = strtoll(p, &end, 16);
